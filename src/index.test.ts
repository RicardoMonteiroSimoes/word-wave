...
describe('WordWaveEngine', () => {
  ...
  describe('backward compatibility', () => {
    ...
    it('constructs with empty effects array (takes legacy path)', () => {
      expect(() => {
        engine = new WordWaveEngine(canvas, { effects: [] });
      }).not.toThrow();
    });
...
describe('Effects system: shader generation', () => {
  describe('generateShaders', () => {
    ...
    it('produces valid shader with empty effects array', () => {
      const { vertexSource, uniformNames } = generateShaders([]);
      expect(vertexSource).toContain('#version 300 es');
      expect(vertexSource).toContain('vec2 displacement = vec2(0.0);');
      expect(uniformNames).toContain('u_time');
      expect(uniformNames).toContain('u_resolution');
    });

    it('handles glsl effect with no params', () => {
      const effects: Effect[] = [{ type: 'glsl', code: 'd = vec2(1.0, 0.0);' }];
      const { vertexSource, uniformNames } = generateShaders(effects);
      expect(vertexSource).toContain('d = vec2(1.0, 0.0);');
      // Only base uniforms, no effect-specific ones
      expect(uniformNames).toEqual(['u_time', 'u_resolution']);
    });

    it('throws when glsl param collides with reserved uniform', () => {
      const effects: Effect[] = [
        { type: 'glsl', params: { u_time: 1.0 }, code: 'd = vec2(0.0);' },
      ];
      expect(() => generateShaders(effects)).toThrow(
    ...
    it('accepts arbitrary glsl code without validation', () => {
      const effects: Effect[] = [
    ...
    it('throws when two glsl effects declare the same param name', () => {
      const effects: Effect[] = [
        ...
        { type: 'glsl', params: { u_freq: 0.1 }, code: 'd = vec2(0.0);' },
      ];
      expect(() => generateShaders(effects)).toThrow(
        /Duplicate effect uniform "u_freq"/,
      );
    });
  });
