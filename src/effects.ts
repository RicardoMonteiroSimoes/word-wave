 ...
 *
 * **Warning — trusted input only.** The `code` string is interpolated
 * directly into the vertex shader source without sanitization. Never
 * pass user-supplied or untrusted strings as `code` — doing so can
 * break the shader, stall the GPU, or read from any shader variable.
 *
 * @example
 * ```typescript
 * {
...
export interface GlslEffect {
   ...
   *
   * **Treated as trusted input** — never pass untrusted strings here.
   */
  code: string;
}
